...
class OrangeGhost(Ghost):
    def __init__(self, canvas, x, y, pacman, interface):
        super().__init__(canvas, x, y, pacman, interface, "orange")

    def get_next_position(self):
        """Clyde-style: si proche (<=8 cases) se dirige vers coin de scatter, sinon poursuit Pac-Man."""
        start = (self.x // 30, self.y // 30)
        pac_col = self.pacman.x // 30
        pac_row = self.pacman.y // 30

        # distance manhattan en cases
        dist = abs(start[0] - pac_col) + abs(start[1] - pac_row)

        max_cols = len(self.pacman.murs[0]) - 1
        max_rows = len(self.pacman.murs) - 1

        # scatter target (coin bas-gauche)
        scatter = (0, max_rows)

        if dist <= 8:
            # trop proche: se dirige vers le coin de scatter
            end = scatter
        else:
            # loin: poursuit Pac-Man
            end = (pac_col, pac_row)
